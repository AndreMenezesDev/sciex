...
{
	public class HistoricoProcessoBll : IHistoricoProcessoBll
	{
		private readonly IUnitOfWorkSciex _uowSciex;
		private readonly IUsuarioPssBll _usuarioPssBll;

		public HistoricoProcessoBll(IUnitOfWorkSciex uowSciex, IUsuarioPssBll usuarioPssBll)
		{
			_uowSciex = uowSciex;
		...
		public PagedItems<PRCSolicHistoricoVM> ListarPaginado(PRCStatusVM objeto)
		{
			...
			string sort = null;
			if (!string.IsNullOrEmpty(objeto.Sort) && objeto.Sort.Equals("DescricaoPlanoExportacao"))
			{
			...
			if(pagedItems.Total > 0)
			{
				...
				{
					item.DescricaoPlanoExportacao = item.NumeroPlano?.ToString("D5") + "/" + item.AnoPlano;
					item.DescricaoStatus = getDescriptionStatus(item.Tipo);
					item.AnoNumeroSolicitacaoFormatado = item.SolicitacaoAlteracao != null ? Convert.ToInt32(item.SolicitacaoAlteracao.NumeroSolicitacao).ToString("D5")  + "/" + Convert.ToInt32(item.SolicitacaoAlteracao.AnoSolicitacao).ToString() : "-";
				}
