...
import { manterTipoDeclaracaoVM } from '../../../view-model/ManterTipoDeclaracaoVM';

@Component({
	selector: 'app-manter-tipo-declaracao-formulario',
	templateUrl: './formulario.component.html'
})

export class ManterTipoDeclaracaoFormularioComponent implements OnInit {
	parametros: manterTipoDeclaracaoVM = new manterTipoDeclaracaoVM();
	model: manterTipoDeclaracaoVM = new manterTipoDeclaracaoVM();
	path: string;
	titulo: string;
	tituloPanel: string;
	desabilitado: boolean;
	servicoTipoDeclaracao = 'TipoDeclaracao';
	servicoTipoDeclaracaoGrid = 'TipoDeclaracaoGrid';
	...
	public selecionar(id: number) {
		if (!id) { return; }
		this.applicationService.get<manterTipoDeclaracaoVM>(this.servicoTipoDeclaracao, id).subscribe(result => {
			this.model = result;
	...
	public salvar() {
		...
		if (this.model.descricao.length != 0) {
			if (this.codigo1 != "" && this.model.descricao != "") {
				...
				this.applicationService.get(this.servicoTipoDeclaracaoGrid, this.parametros).subscribe((result: PagedItems) => {
	...
	private salvarRegistro() {
		...
		this.applicationService.put<manterTipoDeclaracaoVM>(this.servicoTipoDeclaracao, this.model).subscribe(result => {
			if (result.mensagemErro != null) {
				this.modal.alerta(this.msg.REGISTRO_JA_VINCULADO, "Alerta", "/manter-tipo-declaracao");
			}
			else {
				this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO, "Sucesso", "/manter-tipo-declaracao");
				this.model = result;
			}
			if (this.path != "editar")
				localStorage.clear();
	...
	public cancelarOperacao() {
		this.modal.confirmacao(this.msg.CONFIRMAR_CONFIRMACAO, '', '')
			.subscribe(isConfirmado => {
				if (isConfirmado) {
					this.router.navigate(['/manter-tipo-declaracao']);
				}
			});
	}
}
