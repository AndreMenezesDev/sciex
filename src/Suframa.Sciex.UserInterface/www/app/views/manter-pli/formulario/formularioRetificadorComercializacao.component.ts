...
@Component({
	selector: 'app-manter-pli-formulario-retificador-comercializacao',
	templateUrl: './formularioRetificadorComercializacao.component.html'
})

export class ManterPliFormularioRetificadorComercializacaoComponent implements OnInit {
	path: string;
	titulo: string;
	tituloPanel: string;
	...
	ngOnInit() {
		...
		if (localStorage.getItem('manter-pli') != null) {
			...
			this.applicationService.get<manterPliVM>(this.servicoPli, this.modelPli.idPLI).subscribe(result => {
				...
				this.descricaoTipoPli = "RETIFICADOR"; // modificar para modo dinamico
			});
	...
	public verificarRota() {
		...
		if (this.path == 'visualizarretificadoracomercializacao') {
		...
		else if (this.path == 'cadastrarretificadoracomercializacao') {
			this.selecionar(this.route.snapshot.params['id']);
			this.titulo = 'Cadastrar';
			this.mostrarBotao = true;
		}
		else if (this.path == 'editarretificadoracomercializacao') {
			this.selecionar(this.route.snapshot.params['id']);
			this.titulo = 'Alterar';
			this.mostrarBotao = true;
	...
	adicionarMercadoria() {
		...
		this.modelPliMercadoria.idPLI = this.modelPli.idPLI;
	...
	public alterarMercadoria(item){
		let obj = {codigoNCM: item.codigoNCMMercadoria, descricao: item.descricaoNCMMercadoria};
		this.parametros.idPliMercadoria = item.idPliMercadoria;
		this.applicationService.get<manterNcmVM>("ConsultarNcm", obj).subscribe(result  => {
			this.parametros.idMercadoria = result.idNcm;
		});
	}
