...
@Component({
	selector: 'app-modal-mercadoria-pli',
	templateUrl: './modal-mercadoria-pli.component.html',
})

export class ModalMercadoriaPliComponent implements OnInit, OnDestroy {
	//parametros: manterPliVM = new manterPliVM();
	parametros: any = {};
	parametrosCra: any = {};
	...
	@ViewChild('itemMercadoria') itemMercadoria;
	...
	inicializarItensTela() {
		...
		this.applicationService.get("UsuarioLogado", { cnpj: true }).subscribe((result: any) => {
			if (result != undefined) {
				...
				//if (this.model.tipoOrigem != null && this.model.tipoOrigem != undefined && this.model.tipoOrigem == 3) {
				...
				//}
				//else {
				//	this.carregarDadosFornecedor(this.model.idFornecedor);
				//	this.carregarDadosFabricante(this.model.idFabricante);
				//}
	...
	validarItensMercadoria() {
		...
		this.itemMercadoria.valorInput.nativeElement.setCustomValidity('');
		...
		if (this.idDetalheMercadoria == null) {
			this.itemMercadoria.valorInput.nativeElement.setCustomValidity('Preencha este campo.');

		} else if (this.unidadeComercializada.valorInput.nativeElement.value == "") {

			this.unidadeComercializada.valorInput.nativeElement.setCustomValidity('Preencha este campo.');

		} else if (this.quantidadeUnidadeComercializada.nativeElement.value == "" ||
			parseFloat(this.quantidadeUnidadeComercializada.nativeElement.value.replace(",", ".")) == 0) {

			this.quantidadeUnidadeComercializada.nativeElement.setCustomValidity('Preencha este campo.');

		} else if (this.valorUnitarioCondicaoVenda.nativeElement.value == "" ||
			parseFloat(this.valorUnitarioCondicaoVenda.nativeElement.value.replace(",", ".")) == 0) {
	...
	incluirItemMercadoria() {
		...
		//if (!this.formularioDetalhe.valid) { return; }

		if (this.modelDetalheMercadoria.idUnidadeMedida != undefined || this.idUnidadeMedida != undefined) {
			...
			this.modelDetalheMercadoria.descricaoUnidadeMedida = this.modelDetalheMercadoria.siglaUnidadeMedida = null;
		}
		this.modelDetalheMercadoria.descricaoDetalhe = this.itemMercadoria.valorInput.nativeElement.value.split('|')[1].trim();
		this.modelDetalheMercadoria.codigoDetalheMercadoria = +this.itemMercadoria.valorInput.nativeElement.value.split('|')[0].trim();
