...
@Component({
    selector: 'app-manter-parametros',
	templateUrl: './manter-parametros.component.html'

})

@Injectable()
export class ManterParametrosComponent implements OnInit {
    grid: any = { sort: {} };
    parametros: any = {};
    ocultarFiltro: boolean = false;
	ocultarGrid: boolean = false;
	servicoParametrosGrid = 'ParametrosGrid';
	@ViewChild('IdParametro') IdParametro;
	...
	ngOnInit(): void {
		...
		if (this.parametros != undefined || this.parametros != null) {
			...
			if (this.parametros.IdParametro != "-1")
				localStorage.removeItem(this.router.url);
			this.listar();
		}
		else {
			this.parametros = {};
			this.parametros.titulo = "PARÂMETROS";
			this.parametros.width = { 0: { columnWidth: 250 }, 1: { columnWidth: 510 } };
			this.parametros.servico = this.servicoParametrosGrid;
			this.parametros.columns = ["Código", "Descrição"];
			this.parametros.fields = ["idParametro|formatCode:3", "descricao"];
		}

		this.applicationService.get("UsuarioLogado", { cnpj: true }).subscribe((result: any) => {
			if (result != null) {
				this.CNPJ = result;
				this.parametros.CPNJImportador = this.CNPJ;
			}
		});
    ...
    limpar() {
		//this.parametros = {};
		this.descricao.nativeElement.value = this.IdParametro.nativeElement.value = "";
		this.IdParametro.nativeElement.value = "";
	}
	...
	listar() {
		...
		this.applicationService.get(this.servicoParametrosGrid, this.parametros).subscribe((result: PagedItems) => {
