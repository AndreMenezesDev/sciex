...
import { manterNaladiVM } from '../../../view-model/ManterNaladiVM';
...
@Component({
	selector: 'app-manter-naladi-formulario',
	templateUrl: './formulario.component.html'
})

export class ManterNaladiFormularioComponent implements OnInit {
	parametros: manterNaladiVM = new manterNaladiVM();
	path: string;
	titulo: string;
	tituloPanel: string;
	desabilitado: boolean;
	servicoNaladi = 'Naladi';
	servicoNaladiGrid = 'NaladiGrid';
	habilitarCampoCodigo: boolean;
	validar: boolean = false;
	model: manterNaladiVM = new manterNaladiVM();
	codigo1: string;
		...
		if (this.codigo.nativeElement.value != undefined)
			if (this.codigo.nativeElement.value.length != undefined)
				if (this.codigo.nativeElement.value.length > 0) {
						...
						this.codigo1 = ("00000000" + this.codigo1).slice(-8);
					}
	}
	...
	public selecionar(id: number) {
		...
		if (!id) { return; }
		this.applicationService.get<manterNaladiVM>(this.servicoNaladi, id).subscribe(result => {
			this.model = result;
	...
	public salvar() {
		...
		if (this.model.descricao.length != 0) {
			if (this.codigo1 != "" && this.model.descricao != "") {
				...
				this.applicationService.get(this.servicoNaladiGrid, this.parametros).subscribe((result: PagedItems) => {
	...
	private salvarRegistro() {
		...
		this.applicationService.put<manterNaladiVM>(this.servicoNaladi, this.model).subscribe(result => {
			this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO, "Sucesso", "/naladi");
			this.model = result;
			if (this.path != "editar")
				localStorage.clear();
		});
