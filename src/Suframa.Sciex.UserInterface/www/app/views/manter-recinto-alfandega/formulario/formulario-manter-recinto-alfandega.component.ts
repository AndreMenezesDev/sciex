...
@Component({
	selector: 'app-manter-formulario-recinto-alfandega',
	templateUrl: './formulario-manter-recinto-alfandega.component.html'
})

export class FormularioManteRecintoAlfandegaComponent implements OnInit {
		...
		if (this.codigo.nativeElement.value != undefined)
			if (this.codigo.nativeElement.value.length != undefined)
				if (this.codigo.nativeElement.value.length > 0) {
						...
						this.codigo1 = ("0000000" + this.codigo1).slice(-2);
				}
	}
	...
	public cancelarOperacao() {
		this.modal.confirmacao(this.msg.CONFIRMAR_CONFIRMACAO, '', '')
			.subscribe(isConfirmado => {
				if (isConfirmado) {
					this.router.navigate(['/manter-recinto-alfandega']);
				}
		});
	}
	...
	salvarRegistro() {
		this.applicationService.post(this.servico, this.parametros).subscribe(result => {
			if (result == 1) {
				this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO, "Alerta", "/manter-recinto-alfandega");
			}
		});
	}
