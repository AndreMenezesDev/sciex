...
@Component({
	selector: 'app-analise-plano-formulario-propriedade-produto-comprovacao',
	templateUrl: './formularioAnalisePropriedadeProdutoComprovacao.component.html'
})

export class AnalisarPlanoFormularioPropriedadeProdutoComprovacaoComponent implements OnInit {
	isCorrecao = false;
	...
	@ViewChild('pais') pais;
	validar: boolean;
	servicoAnalisarDUE = "AnalisarDeclaracaoUnicaExportacao";
	servicoAprovarTodosDUE = "AprovarTodosDUEEmAnalise";
	...
	listarDUE() {
		...
		this.applicationService.get(this.servicoAnalisarDUE, this.parametrosSolic).subscribe((result: any) => {
	...
	confirmarAprovarTodos(){
		this.modal.confirmacao("Todos os documentos comprobatórios serão aprovados, deseja continuar?", 'Confirmar Aprovação Todos', '')
	...
	aprovarTodos(){
		...
		this.applicationService.post(this.servicoAprovarTodosDUE, dados).subscribe((result: any) => {
			if (result.resultado){
				this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO,'Sucesso','').
				subscribe(()=>{
					this.listarDUE();
