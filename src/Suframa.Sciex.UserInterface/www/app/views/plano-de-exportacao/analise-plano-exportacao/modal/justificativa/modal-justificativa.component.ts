...
@Component({
	selector: 'app-modal-justicar-glosa',
	templateUrl: './modal-justificativa.component.html',
})

export class ModalJustificaGlosaQuadroExportacaooComponent {
	...
	public abrir(dados,formPai) {
		this.formPai = formPai;
		this.parametros = dados;
		this.parametros.descricaoJustificativaErro=null ;
	...
	public salvar() {
		...
		if (this.parametros.descricaoJustificativaErro == null){
			this.modal.alerta(this.msg.CAMPO_$_OBRIGATORIO_NAO_INFORMADO.replace('$','justificativa'));
			return;
		}
		
		this.applicationService.post(this.servicoAnalisar, this.parametros).subscribe((result:any) => {
			if (result.resultado){
				this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO,'Sucesso','').
				subscribe(()=>{
					this.formPai.fecharGlosa();
				});
			}else{
