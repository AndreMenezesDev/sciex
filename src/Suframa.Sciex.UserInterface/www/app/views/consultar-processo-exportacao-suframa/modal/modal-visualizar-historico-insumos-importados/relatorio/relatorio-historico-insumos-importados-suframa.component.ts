...
@Component({
	selector: 'app-relatorio-historico-insumos-importados-suframa',
	templateUrl: './relatorio-historico-insumos-importados-suframa.component.html',
	styleUrls: ['./relatorio-historico-insumos-importados-suframa.component.css']
})

export class RelatorioHistoricoInsumosSuframaComponent{
	...
	emitirHistoricoInsumo(id, isInsumo) {
		...
		let carregaDados = new Promise(resolve => {
			setTimeout(() => {
				this.buscarDados(this.id, isInsumo);
			}, 2000);
			resolve(null);
		}).then(() => {
			setTimeout(() => {
