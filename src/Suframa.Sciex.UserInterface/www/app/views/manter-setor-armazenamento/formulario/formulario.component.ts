...
@Component({
	selector: 'app-manter-formulario-setor-armazenamento',
	templateUrl: './formulario.component.html'
})

export class FormularioSetorArmazenamentoComponent implements OnInit {
    ...
    @ViewChild('dropDownRecinto') dropDownRecinto;
		...
		if (this.codigo.nativeElement.value != undefined)
			if (this.codigo.nativeElement.value.length != undefined)
				if (this.codigo.nativeElement.value.length > 0) {
						...
						this.codigo1 = ("000" + this.codigo1).slice(-2);
				}
	}
	...
	public cancelarOperacao() {
		this.modal.confirmacao(this.msg.CONFIRMAR_CONFIRMACAO, '', '')
			.subscribe(isConfirmado => {
				if (isConfirmado) {
					this.router.navigate(['/manter-setor-armazenamento']);
				}
		});
	}
	...
	public salvar() {
		...
		if(this.path == 'novo' && !this.pesquisouCodigoExistente){
			this.verificarCodigoExistente(Number(this.parametros.codigo), Number(this.parametros.idRecintoAlfandega));
		} else {
			this.salvarRegistro();
		}		
	}
	...
	salvarRegistro() {
		this.applicationService.post(this.servico, this.parametros).subscribe(result => {
			if (result == 1) {
				this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO, "Alerta", "/manter-setor-armazenamento");
			}
		});
	}

	verificarCodigoExistente(codigo:number, idRecintoAlfandega: number){
		var objeto : any = {};
		objeto.codigo = codigo;
		objeto.idRecintoAlfandega = idRecintoAlfandega;	
		this.applicationService.get(this.servico, objeto).subscribe((result : any) => {
			if(result) {
				this.modal.confirmacao('O setor e recinto já foram cadastrados. Deseja alterar o registro?', '', '')
				.subscribe(isConfirmado => {
					if (isConfirmado) {
