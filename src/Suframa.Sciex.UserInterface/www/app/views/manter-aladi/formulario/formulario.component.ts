...
import { manterAladiVM } from '../../../view-model/ManterAladiVM';
...
import { Select2CustomComponent } from '../../../shared/components/select2-custom/select2-custom.component';

@Component({
	selector: 'app-manter-aladi-formulario',
	templateUrl: './formulario.component.html'
})

export class ManterAladiFormularioComponent implements OnInit {
	parametros: manterAladiVM = new manterAladiVM();
	path: string;
	titulo: string;
	tituloPanel: string;
	...
	model: manterAladiVM = new manterAladiVM();
	codigo1: string;
		...
		if (this.codigo.nativeElement.value != undefined)
			if (this.codigo.nativeElement.value.length != undefined)
				if (this.codigo.nativeElement.value.length > 0) {
						...
						this.codigo1 = ("000" + this.codigo1).slice(-3);
					}
	}
	...
	public selecionar(id: number) {
		if (!id) { return; }
		this.applicationService.get<manterAladiVM>(this.servicoAladi, id).subscribe(result => {
			this.model = result;
	...
	private salvarRegistro() {
		...
		this.applicationService.put<manterAladiVM>(this.servicoAladi, this.model).subscribe(result => {
			this.modal.resposta(this.msg.OPERACAO_REALIZADA_COM_SUCESSO, "Sucesso", "/aladi");
			this.model = result;
			if (this.path != "editar")
				localStorage.clear();
		});
