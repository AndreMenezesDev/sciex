...
export const MONEY: Provider = {
	...
	useExisting: forwardRef(() => FormatarNumeroDecimal7Directive),
	multi: true
};

@Directive({
	selector: '[formatar-numero-decimal7]',
	host: {
...
export class FormatarNumeroDecimal7Directive implements ControlValueAccessor {
	...
	writeValue(value: any) {
		...
		if (value === undefined || value === null) {
			...
			if (typeof value[1] == 'undefined') {
				...
				value = value[0] + '.' + value[1].slice(0, 7);
			}
	...
	input(val, event?) {
		...
		const dec = unmask.slice(unmask.length - 7, unmask.length);
		const mil = unmask.slice(0, unmask.length - 7);
