...
{
	public class PliMercadoriaEntityProfile : Profile
	{
		public PliMercadoriaEntityProfile()
		{
			CreateMap<PliMercadoriaEntity, PliMercadoriaVM>()
				.ForMember(dest => dest.CodigoProdutoConcatenado, opt => opt.MapFrom(src => src.CodigoProduto.HasValue ? src.CodigoProduto.Value.ToString("D4") + src.CodigoTipoProduto.Value.ToString("D3") + src.CodigoModeloProduto.Value.ToString("D4") : ""))
				.ForMember(dest => dest.CodigoProdutoFormatado, opt => opt.MapFrom(src => src.CodigoProduto.HasValue ? src.CodigoProduto.Value.ToString("D4") : ""))
				.ForMember(dest => dest.TipoProdutoFormatado, opt => opt.MapFrom(src => src.CodigoTipoProduto.HasValue ? src.CodigoTipoProduto.Value.ToString("D3") : ""))
				.ForMember(dest => dest.ModeloProdutoFormatado, opt => opt.MapFrom(src => src.CodigoModeloProduto.HasValue ? src.CodigoModeloProduto.Value.ToString("D4") : ""))
				...
				.ForMember(dest => dest.TipoALI, opt => opt.MapFrom(src => src.Pli.TipoDocumento == 1 ? "Normal" : src.Pli.TipoDocumento == 2 ? "Substitutiva" : "Retificadora"))
				...
				.ForMember(dest => dest.NumeroLIRetificador, opt => opt.MapFrom(src => src.NumeroLiRetificador == null ? "-" : src.NumeroLiRetificador.ToString()))
				...
				.ForMember(dest => dest.DataDIFormatada, opt => opt.MapFrom(src => src.Li != null && src.Li.Di != null ? src.Li.Di.DataRegistro.Value.ToShortDateString() : "-"))
				.ForMember(dest => dest.NumeroDI, opt => opt.MapFrom(src => src.Li != null && src.Li.Di != null ? src.Li.Di.NumeroDi.ToString() : "-"))
				.ForMember(dest => dest.UtilizadaDI, opt => opt.MapFrom(src => src.Li != null && src.Li.Di != null ? "Sim" : "Não"))
