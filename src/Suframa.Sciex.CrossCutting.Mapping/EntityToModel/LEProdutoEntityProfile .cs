...
{
	public class LEProdutoEntityProfile : Profile
	{
		public LEProdutoEntityProfile()
		{
			CreateMap<LEProdutoEntity, LEProdutoVM>()
				.ForMember(dest => dest.IsInsumos, opt => opt.MapFrom(src => src.LEInsumo.Any()));
				//.ForMember(dest => dest.NumeroPliConcatenado, opt => opt.MapFrom(src => src.Ano + "/" + src.NumeroPli.ToString("d6")))
